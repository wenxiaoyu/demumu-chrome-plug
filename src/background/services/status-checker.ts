    ...
    try {
      ...
      if (currentStatus !== previousStatus) {
        ...
        if (currentStatus === 'dead') {
          ...
          await this.notificationService.showDeathWarning(daysSinceKnock);
        }
      }
      ...
      if (currentStatus === 'alive' && currentHP <= HP_THRESHOLDS.WARNING && currentHP > 0) {
        ...
        if (hpStatus === 'critical' || hpStatus === 'warning') {
          console.log(`[StatusChecker] HP warning: ${currentHP}`);
          await this.notificationService.showHPWarning(currentHP);
        }
      }
