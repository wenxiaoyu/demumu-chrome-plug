...
export class EmailService {
   ...
   * @returns 渲染后的邮件模板
   */
  prepareEmail(variables: EmailTemplateVariables): EmailTemplate {
    const template = getDeathNotificationTemplate(variables.userName);
  ...
  async sendToContacts(
    ...
    // 准备邮件
    const email = this.prepareEmail(updatedVariables);
