...
import { createStatusCheckAlarm, setupAlarmListeners } from './handlers/alarm-handler';
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.type === 'ADD_CONTACT') {
    contactService.addContact(message.data).then((contact) => {
  ...
  if (message.type === 'UPDATE_CONTACT') {
    ...
    contactService.updateContact(id, updates).then((contact) => {
      sendResponse({ success: true, data: contact });
    }).catch((error) => {
      sendResponse({ success: false, error: error.message });
    });
  ...
  if (message.type === 'GET_DEATH_CONFIG') {
    deathDetectionService.getConfig().then((config) => {
  ...
  if (message.type === 'UPDATE_DEATH_CONFIG') {
    deathDetectionService.updateConfig(message.data).then(async (config) => {
      sendResponse({ success: true, data: config });
    }).catch((error) => {
      sendResponse({ success: false, error: error.message });
    });
  ...
  if (message.type === 'CHECK_DEATH_STATUS') {
    deathDetectionService.checkDeathStatus().then((status) => {
  ...
  if (message.type === 'GET_DEATH_STATUS') {
    deathDetectionService.getCurrentStatus().then((status) => {
      sendResponse({ success: true, data: status });
    }).catch((error) => {
      sendResponse({ success: false, error: error.message });
    });
  ...
  if (message.type === 'TRIGGER_EMAIL_SEND') {
    deathDetectionService.triggerEmailSend().then((success) => {
