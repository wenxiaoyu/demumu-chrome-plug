...
import type { UserData, KnockRecord, DailyStats, EmergencyContact, UserSettings, DeathDetectionConfig } from '../types';
  ...
  async syncUserSettings(): Promise<SyncResult> {
    try {
      ...
      const [language, deathDetectionConfig, customEmailTemplate] = await Promise.all([
        ...
        storage.get<{ subject: string; htmlBody: string; textBody: string }>('customEmailTemplate')
      ]);
      ...
      if (!cloudSettings) {
        ...
        console.log('[SyncService] User settings uploaded to cloud (including email template)');
